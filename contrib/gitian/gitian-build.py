...
gsigs = 'https://codeberg.org/peoplecoin/gitian.sigs.git'
...
def setup():
    ...
    os.chdir('inputs')
    if os.path.isdir('peoplecoin'):
        # Remove the potentially stale peoplecoin dir. Otherwise you might face submodule mismatches.
        subprocess.check_call(['rm', 'peoplecoin', '-fR'])
    subprocess.check_call(['git', 'clone', args.url, 'peoplecoin'])
    os.chdir('..')
...
def rebuild():
    ...
    for i in args.os:
        ...
        infile = 'inputs/peoplecoin/contrib/gitian/gitian-' + tag_name + '.yml'
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'peoplecoin='+args.commit, '--url', 'peoplecoin='+args.url, infile])
        ...
        subprocess.check_call('mv build/out/peoplecoin-*.' + suffix + ' ../out/'+args.version, shell=True)
...
def build():
    ...
    subprocess.check_call(['make', '-C', 'inputs/peoplecoin/contrib/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def verify():
    ...
    for i, v in platforms:
        ...
        subprocess.check_call(['bin/gverify', '-v', '-d', '../sigs/', '-r', args.version+'-'+v[1], 'inputs/peoplecoin/contrib/gitian/gitian-'+v[1]+'.yml'])
    os.chdir(workdir)
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://codeberg.org/peoplecoin/peoplecoin', help='Specify the URL of the repository. Default is %(default)s')
    ...
    os.makedirs('builder/inputs/peoplecoin', exist_ok=True)
    os.chdir('builder/inputs/peoplecoin')
    if args.pull:
